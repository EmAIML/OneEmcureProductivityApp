...
def convert_to_story(text, max_gen_len=512, temp=0.1, top_p=0.9, max_retries=6):
    instruction = (
    "Create a clear and accurate educational narration based ONLY on the slide's extracted text "
    "and image description. Follow these rules strictly:\n"
    "1. For content-heavy educational slides, generate a detailed narration that covers every real "
    "point from the text and explains the image meaningfully without adding extra information.\n"
    "2. For introduction, title, index, or minimal-information slides, keep the narration short and "
    "simple. Do not over-explain.\n"
    "3. For text-only slides, read the text naturally and in order, with no extra interpretation.\n"
    "4. Pronounce chemical names, generic names, and scientific terms correctly and naturally.\n"
    "5. Pronounce decimals and percentages exactly as humans speak them. Examples: "
    "0.04 → 'zero point zero four', 4.8502 → 'four point eight five zero two',65.236 → 'sixty five point two three six', "
    "10% → 'ten percent', 89% → 'eighty nine percent'. Do not round or change numbers.\n"
    "6. Do NOT hallucinate or invent facts beyond what the text and image clearly show.\n"
    "7. Maintain a clear educational tone without intros like 'In this slide' and without conclusions.\n\n"
)
...
def add_ssml_tags(text, pause_duration_ms=3000):
    def normalize_pronunciation(txt):
        # Ensure decimals are spoken smoothly like "zero point zero four"
        txt = re.sub(r'(\d)\.(\d)', r'\1 point \2', txt)
        # Add 'percent' word clarity
        txt = re.sub(r'(\d+)\s*%', r'\1 percent', txt)
        # Common chemical/compound patterns (e.g., NaCl, H2O)
        txt = re.sub(r'([A-Z][a-z]?)(\d+)', r'\1 \2', txt)
        txt = re.sub(r'\b([A-Z][a-z]?[A-Z0-9]+)\b', r'\1', txt)
        return txt
    ...
    normalized_text = normalize_pronunciation(text)
    text_with_slide_breaks = insert_slide_breaks_before(normalized_text)
    ...
    ssml = f'<speak><prosody rate="85%">{final_ssml_text}</prosody></speak>'
    return ssml
...
#  Updated main() with no slide limit
def main(txt_file, pause_ms=3000, max_valid_slide=None):
    ...
    if max_valid_slide is not None:
        ...
        print("Processing all slides.")
        valid_sections = slide_sections

    chunk_files = []
    with tempfile.TemporaryDirectory() as tempdir:
        for batch_idx, slide_batch in enumerate(group_slides(valid_sections, batch_size=1)):
            print(f"\n Generating story for Slide {batch_idx + 1}/{len(valid_sections)}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 4:
        print("Usage: python model2.py <input.txt> [pause_ms] [max_slide]")
        sys.exit(1)

    input_file = sys.argv[1]
    pause_duration = int(sys.argv[2]) if len(sys.argv) >= 3 else 3000
    max_slide_number = int(sys.argv[3]) if len(sys.argv) == 4 else None
